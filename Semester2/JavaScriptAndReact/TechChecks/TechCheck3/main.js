...
//array[n].languages.name === "English"
//array[n].borders === "CHN"
//array[n].name === Country Name
...
(function(test){
    ...
    function getPopulation(json){
        const countryData = json
            .filter(country => country.borders)
            .filter(country => country.borders.includes('CHN'))
            .filter(country => country.languages.some(languages => languages.name === 'English'));

        const totalPop = countryData.reduce((total, country) => total + country.population, 0);
        return totalPop;
    }
})(test);